...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
      <ul class="navbar-nav">
        ...
        <li class="nav-item"><a class="nav-link" href="{{ route('fluid-oils.index') }}">الزيوت والسوائل</a></li>
  ...
  <!-- ✅ عنوان الصفحة -->
  <div class="container">
    <h2 class="header-title">جميع الزيوت والسوائل</h2>

    <!-- ✅ فلتر النوع -->
    <div class="filter-section">
      <form method="GET" action="{{ route('fluid-oils.index') }}">
    ...
    <div class="parts-grid">
      @forelse($fluidOils as $fluidOil)
        <div class="part-card">
          <img src="{{ asset('storage/' . $fluidOil->image) }}" alt="{{ $fluidOil->name }}">
          <h5>{{ $fluidOil->name }}</h5>
          <p>{{ $fluidOil->description }}</p>
          <a href="{{ route('fluid-oils.show', $fluidOil->id) }}" class="btn btn-warning btn-sm mt-2">تفاصيل</a>
        </div>
      @empty
        <p class="text-center w-100">لا توجد زيوت أو سوائل متوفرة حالياً.</p>
      @endforelse
    </div>
  </div>
