            ...
            <div class="row">
                <div class="col-md-4 text-center">
                    @if ($suspensionPart->image)
                        <img src="{{ asset('storage/' . $suspensionPart->image) }}" alt="صورة القطعة" class="img-fluid rounded mb-4">
                    @else
                ...
                <div class="col-md-8">
                    <div class="part-info">
                        <p><strong>اسم الشركة المصنعة:</strong> <span class="text-muted">{{ $suspensionPart->manufacturer }}</span></p>
                    </div>
                    <div class="part-info">
                        <p><strong>السعر:</strong> <span class="text-danger">{{ $suspensionPart->price }} ريال</span></p>
                    </div>
                    <div class="part-info">
                        <p><strong>الوصف:</strong> <span class="text-muted">{{ $suspensionPart->description ?? 'لا يوجد وصف' }}</span></p>
                    </div>

                    <div class="mt-4">
                        <!-- زر تعديل القطعة -->
                        <a href="{{ route('suspension-parts.edit', $suspensionPart->id) }}" class="btn btn-primary btn-lg mr-2">تعديل</a>

                        <!-- نموذج الحذف -->
                        <form action="{{ route('suspension-parts.destroy', $suspensionPart->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                            @csrf
                            @method('DELETE')
                        ...
                        <!-- زر العودة للقائمة -->
                        <a href="{{ route('suspension-parts.index') }}" class="btn btn-secondary btn-lg mt-2">رجوع</a>
                    </div>
                </div>
            </div>
        </div>
