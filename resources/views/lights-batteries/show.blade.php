...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="collapse navbar-collapse justify-content-end">
      <ul class="navbar-nav">
        ...
        <li class="nav-item"><a class="nav-link" href="{{ route('lights-batteries.index') }}">الإضاءة والبطاريات</a></li>
  ...
  <div class="container my-5">
    <h2 class="text-center">{{ $lightBattery->name }}</h2>

    <div class="row">
      <div class="col-md-6">
        <img src="{{ asset('storage/' . $lightBattery->image) }}" class="img-fluid" alt="{{ $lightBattery->name }}">
      </div>
      <div class="col-md-6">
        <h5>النوع: {{ $lightBattery->type }}</h5>
        <p>{{ $lightBattery->description }}</p>
        <p><strong>السعر:</strong> {{ $lightBattery->price }} ر.س</p>
        <a href="{{ route('lights-batteries.index') }}" class="btn btn-primary">العودة إلى القائمة</a>
      </div>
    </div>
  </div>
