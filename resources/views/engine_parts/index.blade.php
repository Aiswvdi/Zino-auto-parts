...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
      <ul class="navbar-nav">
        ...
        <li class="nav-item"><a class="nav-link" href="{{ route('engine_parts.index') }}">قطع المحرك</a></li>
  ...
  <!-- ✅ عنوان الصفحة -->
  <div class="container">
    <h2 class="header-title">جميع قطع المحرك</h2>

    <!-- ✅ فلتر النوع -->
    <div class="filter-section">
      <form method="GET" action="{{ route('engine_parts.index') }}">
    ...
    <div class="parts-grid">
      @forelse($engineParts as $part)
        <div class="part-card">
          <img src="{{ asset('storage/' . $part->image) }}" alt="{{ $part->name }}">
          <h5>{{ $part->name }}</h5>
          <p>{{ $part->description }}</p>
          <a href="{{ route('engine_parts.show', $part->id) }}" class="btn btn-warning btn-sm mt-2">تفاصيل</a>
        </div>
      @empty
